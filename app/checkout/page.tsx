...
function CheckoutContent() {
  ...
  useEffect(() => {
    async function fetchCheckoutData() {
      // 1. Ambil tajuk album dari Supabase menggunakan albumId
      if (albumId) {
      ...
      // 2. Ambil senarai ID foto yang dipilih dari sessionStorage
      const savedIds = sessionStorage.getItem('checkout_photos');
      if (savedIds) {
        const photoIds: string[] = JSON.parse(savedIds);
        if (photoIds.length > 0) {
          // Ambil maklumat foto (URL & BIB) dari Supabase berdasarkan ID
          ...
          if (photosData) {
            ...
            setSelectedPhotoItems(formatted);
          }
        }
      }
    }
            ...
            <div className="pt-2">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedPhotoItems.map((photo) => (
                  <div key={photo.id} className="relative w-14 h-14 rounded-lg overflow-hidden border border-zinc-800 bg-zinc-900">
