        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {filteredPhotos.map((photo) => {
              ...
              return (
                <div key={photo.id} onClick={() => toggleSelectPhoto(photo.id)} className={`group relative rounded-2xl overflow-hidden cursor-pointer border transition-all ${isSelected ? 'border-amber-500 ring-2 ring-amber-500/50 scale-[1.02]' : 'border-zinc-900 hover:border-zinc-700'}`}>
                  <div className="relative aspect-[4/3] bg-zinc-900 overflow-hidden select-none">
                    ...
                    <div className={`absolute top-3 right-3 w-7 h-7 rounded-lg flex items-center justify-center transition ${isSelected ? 'bg-amber-500 text-black' : 'bg-black/60 border border-white/20 text-transparent'}`}>
                      <Check className="w-4 h-4 stroke-[3]" />
                    </div>
                    <span className="absolute bottom-3 left-3 bg-black/80 backdrop-blur-md px-2.5 py-1 rounded-md text-[10px] font-bold text-amber-500 border border-zinc-800">BIB #{photo.bib}</span>
                  </div>
